...
const App: React.FC = () => {
  ...
  const stats = useMemo(() => {
    if (habits.length === 0) return { streak: 0, actualDone: 0, rate: 0, missed: 0 };
    
    const actualDone = Object.values(completions).reduce((t, day) => 
      t + Object.values(day).filter(Boolean).length, 0
    );

    const daysPassedInMonth = Math.min(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).getDate(), 30);
    const totalPossibleSoFar = habits.length * daysPassedInMonth;
    ...
    for (let i = 0; i < 30; i++) {
        ...
        if (dayCompletions && Object.values(dayCompletions).filter(Boolean).length >= Math.ceil(habits.length / 2)) {
            currentStreak++;
        } else if (i === 0) {
            continue;
        } else {
    ...
    return { streak: currentStreak, actualDone, rate, missed: Math.max(totalPossibleSoFar - actualDone, 0) };
  ...
  return (
    <div className="min-h-screen bg-bg-main text-text-main font-sans overflow-x-hidden selection:bg-primary/30">
      <main className="max-w-[1920px] mx-auto flex flex-col min-w-0 h-screen">
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-8 pb-8 space-y-8 no-scrollbar">
          {/* Dashboard Stats Bar */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 py-4 mt-4">
             <div className="card !p-6 flex items-center gap-6 group">
                ...
                <div>
                   <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-1">Current Streak</p>
                   <p className="text-2xl font-black text-white">{stats.streak} <span className="text-xs text-white/40 uppercase tracking-widest font-bold ml-1">Days</span></p>
                </div>
             </div>
             <div className="card !p-6 flex items-center gap-6 group">
                ...
                <div className="flex-1">
                   <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-1">Month Progress</p>
             ...
             <div className="card !p-6 flex items-center gap-6 group">
                ...
                <div>
                   <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-1">Total Habits Done</p>
             ...
             <div className="card !p-6 flex items-center gap-6 group">
                ...
                <div>
                   <p className="text-[10px] font-black uppercase tracking-[0.2em] text-white/40 mb-1">Active Habits</p>
          ...
          <div className="flex flex-col xl:flex-row gap-8 items-start">
            ...
            <div className="w-full xl:w-96 flex-shrink-0">
              <AnalysisPanel habits={habits} completions={completions} month={currentMonthName} />
            </div>
          </div>

          <MonthlyProgress habits={habits} completions={completions} month={currentMonthName} />
          <WeeklyStats habits={habits} completions={completions} currentDate={currentDate} />
        </div>
